...
---
## ROUND R19 — Exam notification: notes whose exam is missing are reported (student form + admin home + notification text) and filled in automatically when the exam is uploaded. Code by Claude; do NOT change code.
Files (add ONLY these): vps-api-v2/api.js, vps-api-v2/test/selftest.js, src/lib/api.ts, src/components/ExamNotificationForm.tsx, src/components/admin/AdminHero.tsx, HANDOFF_VPS_MIGRATION.md
Raw output into `R19_REPORT.md` (not committed). No force-push. Do not touch production data.
1. `cd vps-api-v2 && node test/selftest.js` → 3 lines "... tests OK" + "ALL TESTS PASSED".
2. Root: `npm run lint` and `npm run build` → both pass.
