...
(async () => {
  ...
  // ---------- exam notifications (Add Notification for Exam) ----------
  {
    ...
    S.saveRow('library', { id: 'fIdioms', title: 'Idioms 600', type: 'folder', isFolder: 'true' });
    S.saveRow('library', { id: 'noteIdm', title: '151-175', type: 'note', parentId: 'fIdioms' });
    S.saveRow('library', { id: 'exSyn', title: 'Syno 400 from 151-175', type: 'exam' });
    S.saveRow('library', { id: 'exIdm', title: 'Idioms 600 from 151-175', type: 'exam', parentId: 'fIdiomsExam' });
    ...
    S.saveRow('batches', { id: 'B2x', name: 'Shonibar Sakal', assignedItemsMap: { note1: day1, ex2: day1, ex3: day1, p1: day1, noteIdm: day1 }, scheduledStartTimeMap: { ex3: '2026-01-01T00:00:00.000Z' } });
    ...
    assert.deepEqual(r.data.data.exams.map(x => x.id).sort(), ['ex1', 'ex2', 'exIdm']);
    ...
    r = await call('apiCreateExamNotification', [{ batchId: 'B2x', examDate: D, examIds: ['ex1', 'ex2', 'exIdm'] }], st2);
    ...
    assert.deepEqual(JSON.parse(r.data.data.examIds), ['p2', 'c1', 'j1', 'ex1', 'ex2', 'exIdm']);
