...
const payload = JSON.stringify({ action: 'apiHealStudentIds', args: [] });
...
console.log("Triggering Google Sheets self-healing for student IDs...");
    ...
    const redirectReq = http.request(redirectUrl, { method: 'GET' }, (redirectRes) => {
      ...
      redirectRes.on('end', () => {
        try {
          ...
          console.log("Healing Result:", JSON.stringify(parsed, null, 2));
        } catch(e) {
